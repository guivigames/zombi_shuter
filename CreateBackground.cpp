...
int createBackground(sf::VertexArray& rVA, sf::IntRect arena)
{
    ...
    for (int w = 0; w < worldWidth; w++)
    {
        ...
        {
            // position each vertex in the curretnt quad
            ...
            if (h == 0 || h == worldHeight - 1 || 
                w == 0 || w == worldWidth - 1)
            {
                rVA[currentVertex + 0].texCoords = sf::Vector2f( 0, 0 + TILE_TYPE * TILE_SIZE);
                rVA[currentVertex + 1].texCoords = sf::Vector2f( TILE_SIZE, 0 + TILE_TYPE * TILE_SIZE);
                rVA[currentVertex + 2].texCoords = sf::Vector2f( TILE_SIZE, TILE_SIZE + TILE_TYPE * TILE_SIZE);
                rVA[currentVertex + 3].texCoords = sf::Vector2f( 0, TILE_SIZE + TILE_TYPE * TILE_SIZE);
            }
            else 
            {
                // Use the random floor tile.
            ...
            // Positioin ready for the next four vertices.
